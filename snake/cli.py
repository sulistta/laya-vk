...
def play(argv=None):
    ...
    parser.add_argument("--subfolder", help="Checkpoint subfolder inside a bundled Hub repo")
    parser.add_argument(
        "--backend",
        choices=("auto", "laya", "planner"),
        default="auto",
        help="auto/laya: Vulkan encoder when available, else CPU torch; planner: no model",
    )
    parser.add_argument(
        "--device",
        help="Force a torch device (cpu, cuda, mps, vulkan). Default: backend auto",
    )
    ...
    if backend in ("auto", "laya"):
        ...
        if args.model or args.subfolder or args.device:
            parser.error("--model/--subfolder/--device need --backend laya, not planner")
        print("Planner backend: no neural model, following the cycle-safe route...", file=sys.stderr)
        policy = PlannerPolicy(guarded=not args.unassisted, prompt=args.prompt)
    ...
    stats = {
        ...
        "engine": policy.metadata.get("engine", "unknown"),
