...
class Agent:
    ...
    def __init__(
        ...
        # 1. Device resolution: cuda → mps → vulkan → cpu (auto-fallthrough).
        # Explicit device="vulkan" (or "iree") loads the IREE encoder when artifacts
        # exist, otherwise warns and runs fully on CPU torch.
        self.backend = "torch"
        self._vulkan_encoder = None
        want_vulkan = device is not None and str(device).lower() in ("vulkan", "iree")
        if want_vulkan:
            ...
            if torch.cuda.is_available():
                ...
                self.device = torch.device("cpu")
                want_vulkan = True  # auto: try Vulkan before settling on CPU
        ...
        if want_vulkan:
            self._enable_vulkan(explicit=want_vulkan and device is not None)

    def _enable_vulkan(self, *, explicit: bool) -> None:
        """Swap in the hybrid Vulkan encoder; fall back to CPU torch on any failure."""
        try:
            from .vulkan import attach_vulkan_encoder, vulkan_available

            if not vulkan_available():
                raise RuntimeError("IREE Vulkan driver or vmfb artifacts unavailable")
            pad = self.tok.pad_token_id or 0
            self._vulkan_encoder = attach_vulkan_encoder(
                self.model, pad_id=pad, warmup=True
            )
            self.device = torch.device("cpu")
            self.backend = "vulkan"
            shapes = ", ".join(f"B{b}/L{l}" for b, l in self._vulkan_encoder.available_shapes())
            print(f"[laya] Vulkan encoder active ({shapes}); head on CPU torch.", flush=True)
            ...
            if explicit:
                print(
                    f"[laya] Warning: Vulkan unavailable ({reason}). Falling back to CPU.\n"
                    "  Build artifacts with: python scripts/export_vulkan.py",
                    flush=True,
                )
            else:
                print(f"[laya] Vulkan skipped ({reason}); using CPU torch.", flush=True)
